...
namespace LeagueAppApi.Services
{
    public class PlayerRepository : IPlayerRepository
    {
        ...
        {
            return _context.Players.Include(x => x.Club).Include(x => x.Squad);
        }

        public Player GetPlayer(int id)
        {
            return _context.Players.Include(x => x.Club).Include(x => x.Squad).FirstOrDefault(x => x.Id == id);
        }

        public Player AddPlayer(PlayerCreationDto playerDto)
        {

            var parentClub = _context.Clubs.FirstOrDefault(club => club.Id == playerDto.ClubId);
            if (parentClub == null) throw new Exception("Parent club does not exist"); //TODO return error nicely

            var parentSquad = _context.Squads.FirstOrDefault(squad => squad.Id == playerDto.SquadId);
