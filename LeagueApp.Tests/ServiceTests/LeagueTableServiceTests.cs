...
namespace Tests
{
    ...
    public class LeagueTableServiceTests
    {
        ...
        public LeagueTableServiceTests()
        {
            ...
            _leagueTableService = new LeagueTableService(_context);
        ...
        [Test]
        public void CanGenerateLeagueTable()
        {
            var table = _leagueTableService.GenerateTable(_TestSeason);
            //Participant 1 will be first as they've won the most games
            Assert.AreEqual(_Participant1.DisplayName, table.rows.First().TeamDisplayName);
            //They should have 6 points as they have 2 wins
            Assert.AreEqual(6, table.rows.First().Points);
        }
    }
}